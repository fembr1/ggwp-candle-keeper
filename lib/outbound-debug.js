...
function formatHitFailure({ url, status, headers, data, networkError }) {
  const hdrs = pickHeaders(headers);
  const body = sanitizeBody(bodyText(data));
  const cfMitigated = hdrs["cf-mitigated"] || "";
  const cfRay = hdrs["cf-ray"] || "";
  const server = hdrs.server || "";
  const isCloudflare =
    /challenge/i.test(cfMitigated) ||
    body.looksLikeCloudflareChallenge ||
    ((status === 403 || status === 429) && /cloudflare/i.test(server));

  let interpretation;
  if (isCloudflare) {
    interpretation = `Cloudflare blocked or challenged this hit (cf-mitigated=${
      cfMitigated || "n/a"
    }, cf-ray=${cfRay || "n/a"}). Same class of issue as a WebSocket 403 — not a simple expired-token JSON error.`;
  } else if (status === 401 || status === 403) {
    interpretation = `HTTP ${status} without a Cloudflare challenge page. Token is likely expired, invalid, or the session rejected the hit.`;
  } else if (networkError) {
    interpretation = `No HTTP response (${networkError}). Could be a Cloudflare drop, TLS failure, or outbound network — there is no API JSON body to inspect.`;
  } else if (status) {
    interpretation = `HTTP ${status} from the hit endpoint. See headers and body.`;
  } else {
    interpretation = "Hit failed with no status. See the dump below.";
  }
  ...
  return {
    ...
    cloudflare: isCloudflare,
    report: [
      "===== HIT FAILURE (copy this block for DevOps) =====",
      JSON.stringify(diag, null, 2),
      "===== END HIT FAILURE =====",
    ].join("\n"),
  };
