...
class SocketClient extends EventEmitter {
  ...
  resumeSession(sessionId) {
    ...
    st.endTimestampMs = null;
    st.lastRoomEventAt = null;
    st.silenceStage = "none";
  ...
  initializeSocket() {
    ...
    socket.on("disconnect", (reason) => {
      this.connected = false;
      this.socketId = null;
      if (reason !== "io client disconnect") this.resetSilenceStages();
  ...
  resetSilenceStages() {
    const now = Date.now();
    for (const st of this.sessionState.values()) {
      st.silenceStage = "none";
      if (st.lastRoomEventAt != null) st.lastRoomEventAt = now;
    }
  }

  startSilenceWatchdog() {
    if (this.silenceTimer) return;
    this.silenceTimer = setInterval(() => this.checkRoomSilence(), SILENCE_WATCHDOG_MS);
  }
  ...
  maybeFireHitFromDeadline(session, st) {
    const endTimestamp = st.endTimestampMs;
    if (endTimestamp == null) return false;
    const timeDiff = endTimestamp - Date.now();
    st.lastTimeDiff = timeDiff;
    if (timeDiff >= session.maxMsDiff) return false;
    if (st.hitEndTimestamp === endTimestamp) return false;
    st.hitEndTimestamp = endTimestamp;
    this.log("info", "Threshold crossed — starting hit chance...", session.id);
  ...
  checkRoomSilence() {
    ...
    for (const session of this.activeSessions()) {
      const st = this.sessionState.get(session.id);
      if (!st || st.waitingForStart || st.endTimestampMs == null) continue;
      if (st.lastRoomEventAt == null) continue;

      const silentFor = now - st.lastRoomEventAt;
      const pastDeadline =
        st.endTimestampMs <= now && st.hitEndTimestamp === st.endTimestampMs;
      if (pastDeadline) continue;

      if (st.silenceStage === "none" && silentFor >= ROOM_SILENCE_MS) {
        const hit = this.maybeFireHitFromDeadline(session, st);
        this.log(
          "warn",
          `Room stream silent for ${silentFor}ms${
            hit ? " — firing hit from last endTimestamp" : ""
          } — rejoining room`,
          session.id
        );
      ...
      if (
        st.silenceStage === "rejoined" &&
        silentFor >= ROOM_SILENCE_MS * 2
      ) {
        this.maybeFireHitFromDeadline(session, st);
        st.silenceStage = "reconnecting";
        forceReconnect = true;
        this.log(
          "warn",
          "Room stream still silent — forcing reconnect",
          session.id
        );
      }
    }

    if (forceReconnect) this.forceSocketReconnect();
  }

  forceSocketReconnect() {
    const socket = this.socket;
    if (!socket || !this.running) return;
    this.log("info", "Forcing Socket.IO reconnect after silent room stream");
    for (const session of this.activeSessions()) {
      const st = this.sessionState.get(session.id);
      if (st) st.silenceStage = "reconnecting";
    }
    try {
      socket.disconnect();
      socket.connect();
    } catch (err) {
      this.log("error", `Force reconnect failed: ${err.message}`);
    }
  }
  ...
  handleRoomEvent(session, response) {
    ...
    st.endTimestampMs = endTimestamp;
    st.lastRoomEventAt = Date.now();
    st.silenceStage = "none";
  ...
  async executePostRequest(session) {
    ...
    const n = rt.users.length;
    if (n === 0) return;
    const start = this.nextUserIndex % n;

    for (let i = 0; i < n; i++) {
      const fresh = this.sessionState.get(session.id);
      if (
        !this.running ||
        !fresh ||
        fresh.stopped ||
        ...
        this.log("info", "Hit chance aborted — session no longer listening", session.id);
        return;
      }

      const userIndex = (start + i) % n;
      this.nextUserIndex = (userIndex + 1) % n;
      const ok = await this.attemptHit(session, rt, userIndex);
      if (ok) return;
      ...
      if (remaining > 0) {
        this.log(
          "warn",
          `User ${userIndex + 1}/${n} failed — trying next (${remaining} left this chance)`,
          session.id
        );
      }
    }
    ...
    this.log(
      "error",
      `All ${n} token${n === 1 ? "" : "s"} failed this chance — waiting for next countdown`,
      session.id
    );
  }

  async attemptHit(session, rt, userIndex) {
    ...
    try {
      this.log(
        "info",
        `Executing POST as user ${userIndex + 1}/${n}... ${session.apiUrl}`,
        session.id
      );
